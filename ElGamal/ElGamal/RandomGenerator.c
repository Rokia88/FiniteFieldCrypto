...
void generate_prime_number_0(mpz_t alea, unsigned int nbr_bit)
{
    ...
    do
    {
        mpz_urandomb(alea,generator, nbr_bit);
        prime = mpz_probab_prime_p(alea,10);
    }while(prime==0);
}

//generate a prime number of at most k_bit s.t. p = 2*alea + 1
void generate_sophie_prime_number(mpz_t alea, mpz_t p, unsigned int nbr_bit)
{
    ...
    do
    {
        generate_prime_number(alea,nbr_bit);
        mpz_mul_ui(val2,alea,2);
        mpz_add_ui(val2,val2,1);
    }while(mpz_cmp(p, val2)!= 0);
    
    mpz_clears(val2,NULL);
}
